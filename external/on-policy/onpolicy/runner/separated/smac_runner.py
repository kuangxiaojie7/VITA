...
class SMACRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            def _mean_agent_metric(key: str) -> float:
                vals = [train_infos[a].get(key, 0.0) for a in range(self.num_agents)]
            ...
            json_metrics = {
                ...
                "episode_reward": float(np.mean(self.buffer[0].rewards)) if self.buffer else 0.0,
                "policy_loss": _mean_agent_metric("policy_loss"),
                "value_loss": _mean_agent_metric("value_loss"),
                "dist_entropy": _mean_agent_metric("dist_entropy"),
                "entropy": _mean_agent_metric("dist_entropy"),
            }
