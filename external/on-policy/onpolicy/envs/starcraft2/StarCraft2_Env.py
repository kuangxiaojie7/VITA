...
class StarCraft2Env(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        try:
            ...
            agent_positions = np.zeros((self.n_agents, 2), dtype=np.float32)
            ...
            for agent_id in range(self.n_agents):
                ...
                if unit is None:
                    continue
                agent_positions[agent_id, 0] = float(getattr(unit.pos, "x", 0.0))
                agent_positions[agent_id, 1] = float(getattr(unit.pos, "y", 0.0))
                alive_mask[agent_id] = 1.0 if float(getattr(unit, "health", 0.0)) > 0 else 0.0
            for i in range(self.n_agents):
                infos[i]["agent_positions"] = agent_positions
                infos[i]["alive_mask"] = alive_mask
