    ...
    def train(self) -> None:
        ...
        for update in range(1, self.train_cfg.updates + 1):
            ...
            for step in range(self.train_cfg.episode_length):
                neighbor_obs_tensor = self._build_neighbor_tensor(obs)
                flat_obs = obs.view(self.num_envs * self.num_agents, self.obs_dim)
                flat_state = (
                    state.unsqueeze(1)
                ...
                actions, log_probs, values, entropy, next_actor, next_critic = self.policy.act(
                    flat_obs, flat_state, flat_actor, flat_critic, flat_masks
                )
